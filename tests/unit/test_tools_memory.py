...
def test_get_session_cycles_returns_recorded_cycles(tmp_db, monkeypatch):
    ...
    asyncio.run(
        record_cycle(
            ...
            cycle_index=0,
            outcome="COMMITTED",
            shortlisted_symbols=["XLK", "SPY"],
            risk_level="LOW",
            abort_reason="",
            abort_stage="",
            trade_id="t-1",
            realised_pnl_pct=1.5,
        )
    )
...
def test_record_cycle_committed(tmp_db, monkeypatch):
    ...
    result = asyncio.run(
        record_cycle(
            ...
            cycle_index=0,
            outcome="COMMITTED",
            shortlisted_symbols=["XLE", "XLF"],
            risk_level="MEDIUM",
            abort_reason="",
            abort_stage="",
            trade_id="t-abc",
            realised_pnl_pct=0.8,
        )
    )
...
def test_record_cycle_aborted(tmp_db, monkeypatch):
    ...
    result = asyncio.run(
        record_cycle(
            ...
            shortlisted_symbols=[],
            risk_level="HIGH",
            abort_reason="risk_HIGH",
            abort_stage="risk_gate",
            trade_id="",
