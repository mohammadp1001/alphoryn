...
def test_register_session_file_persists_row(tmp_db: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    file_id = register_session_file(
        "sess-2", "/reports/sess-2/analysis/SPY_momentum.md", "analysis", "SPY"
    )
    conn = sqlite3.connect(str(tmp_db))
    conn.row_factory = sqlite3.Row
...
def test_get_session_files_db_filter_by_file_type(
...
def test_get_session_files_db_filter_by_symbol(
    tmp_db: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("db.schema._connect", _connect_to(tmp_db))
...
def test_register_session_file_tool_with_symbol(
    tmp_db: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("db.schema._connect", _connect_to(tmp_db))
...
def test_register_session_file_tool_empty_symbol_stores_null(
    tmp_db: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("db.schema._connect", _connect_to(tmp_db))
...
def test_get_session_files_tool_filter_file_type(
    tmp_db: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("db.schema._connect", _connect_to(tmp_db))
...
def test_get_session_files_tool_filter_symbol(
    tmp_db: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("db.schema._connect", _connect_to(tmp_db))
...
def test_get_session_files_tool_empty_strings_act_as_no_filter(
    tmp_db: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("db.schema._connect", _connect_to(tmp_db))
