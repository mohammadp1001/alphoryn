...
def test_etf1_is_required() -> None:
    with pytest.raises(ValidationError):
        AlphorynConfig(etf2="QQQ")  # type: ignore[call-arg]


def test_etf2_is_required() -> None:
    with pytest.raises(ValidationError):
...
def test_session_money_budget_none_accepted() -> None:
    cfg = AlphorynConfig(etf1="SPY", etf2="QQQ", session_money_budget=None)
    assert cfg.session_money_budget is None


def test_session_money_budget_positive_accepted() -> None:
    cfg = AlphorynConfig(etf1="SPY", etf2="QQQ", session_money_budget=1000.0)
...
def test_fractional_session_count_rounds_down() -> None:
    ...
    cfg = AlphorynConfig(etf1="SPY", etf2="QQQ", candle_timeframe="4H", run_duration="25H")
    assert cfg.session_count == math.floor(25 / 4)


def test_non_fractional_session_count() -> None:
    cfg = AlphorynConfig(etf1="SPY", etf2="QQQ", candle_timeframe="1H", run_duration="24H")
...
def test_fractional_run_duration_does_not_raise() -> None:
    ...
    cfg = AlphorynConfig(etf1="SPY", etf2="QQQ", candle_timeframe="4H", run_duration="10H")
...
def test_stop_loss_pct_zero_is_invalid() -> None:
    with pytest.raises(ValidationError):
        AlphorynConfig(etf1="SPY", etf2="QQQ", stop_loss_pct=0.0)


def test_stop_loss_pct_one_is_invalid() -> None:
    with pytest.raises(ValidationError):
        AlphorynConfig(etf1="SPY", etf2="QQQ", stop_loss_pct=1.0)


def test_stop_loss_pct_negative_is_invalid() -> None:
    with pytest.raises(ValidationError):
        AlphorynConfig(etf1="SPY", etf2="QQQ", stop_loss_pct=-0.01)


def test_stop_loss_pct_boundary_low_valid() -> None:
    cfg = AlphorynConfig(etf1="SPY", etf2="QQQ", stop_loss_pct=0.001)
    assert cfg.stop_loss_pct == pytest.approx(0.001)


def test_stop_loss_pct_boundary_high_valid() -> None:
    cfg = AlphorynConfig(etf1="SPY", etf2="QQQ", stop_loss_pct=0.20)
...
def test_load_config_none_cli_override_does_not_clobber_file(tmp_path: Path) -> None:
    cfg_file = tmp_path / "config.json"
    cfg_file.write_text(
        json.dumps({"etf1": "SPY", "etf2": "QQQ", "candle_timeframe": "30min"}),
        encoding="utf-8",
    )
