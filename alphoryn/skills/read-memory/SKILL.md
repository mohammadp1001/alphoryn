---
name: read-memory
description: Summarise recent trade performance for a ticker and strategy combination from the memory entries provided in context, and flag whether a stricter entry signal threshold should apply.
---

**When to use**: At the start of investigation for each ticker, once the regime is identified.
...
- No prior trades: `"No prior {strategy} trades for {ticker} to reference."`
- Adjustment active: `"Recent {strategy} on {ticker}: {incorrect_count}/{total_evaluated} incorrect — requiring stronger entry signal."`
- No adjustment: `"Recent {strategy} on {ticker}: {correct_count}/{total_evaluated} correct."`

## Step 5 — Pass outputs to entry skill

Pass `memory_adjustment` (bool) and `memory_summary` (str) to the relevant entry skill (`mean-reversion-entry` or `momentum-entry`). Include `memory_summary` in the final `reasoning` field of the AssetDecision.
