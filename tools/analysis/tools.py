...
async def compute_atr(
    symbol: str, highs: list[float], lows: list[float], closes: list[float], period: int
) -> dict:
...
async def compute_beta(
    ...
    """
    logger.info(
        "compute_beta symbol=%s benchmark=%s n_returns=%d", symbol, benchmark, len(symbol_returns)
    )
    import numpy as np  # type: ignore[import]
    ...
    if len(x) < 2:
        return BetaResponse(
            symbol=symbol, benchmark=benchmark, beta=1.0, r_squared=0.0, period_days=len(x)
        ).model_dump()
...
async def detect_momentum(symbol: str, closes: list[float], volumes: list[float]) -> dict:
    ...
    if len(closes) < 20:
        return MomentumResponse(
            ...
            price_vs_sma_contribution=0.0,
            volume_trend_contribution=0.0,
            raw_rsi=50.0,
            raw_macd_histogram=0.0,
        ).model_dump()
...
async def detect_crossover(symbol: str, macd_histogram: list[float]) -> dict:
    ...
    if len(macd_histogram) < 2:
        return CrossoverResponse(
            symbol=symbol, crossover_type="none", bars_since_crossover=None, strength=0.0
        ).model_dump()
...
async def run_backtest(
    symbol: str, strategy: str, closes: list[float], volumes: list[float]
) -> dict:
    ...
    if lookback < 20:
        return BacktestResponse(
            symbol=symbol,
            strategy=strategy,
            signal_pattern="insufficient_data",
            lookback_bars=lookback,
            forward_return_days=fwd,
            match_count=0,
    ...
    if not matches:
        return BacktestResponse(
            symbol=symbol,
            strategy=strategy,
            signal_pattern=f"{strategy.lower()}_no_matches",
            lookback_bars=lookback,
            forward_return_days=fwd,
            match_count=0,
            avg_forward_return_pct=0.0,
...
async def score_technical(
    symbol: str, strategy: str, rsi_current: float, macd_histogram: float, pct_b: float
) -> dict:
    ...
    weights = {
        "MOMENTUM": (0.35, 0.40, 0.25),
        "MEAN_REVERSION": (0.25, 0.25, 0.50),
        "SECTOR_ROTATION": (0.30, 0.35, 0.35),
    }.get(strategy, (0.33, 0.33, 0.33))
