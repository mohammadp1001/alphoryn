...
# ---------------------------------------------------------------------------
# Forex tools (OANDA practice account)
# ---------------------------------------------------------------------------

class ForexAccountStatus(_Base):
    account_id: str
    balance: FiniteFloat
    nav: FiniteFloat             # net asset value
    unrealized_pl: FiniteFloat
    margin_used: FiniteFloat
    margin_available: FiniteFloat
    open_position_count: int
    currency: str                # account base currency, e.g. "EUR"
    is_practice: bool


class ForexPositionSide(_Base):
    units: int
    avg_price: FiniteFloat
    unrealized_pl: FiniteFloat


class ForexPosition(_Base):
    instrument: str              # e.g. "EUR_USD"
    long: ForexPositionSide | None = None
    short: ForexPositionSide | None = None
    net_units: int
    unrealized_pl: FiniteFloat


class ForexPositionsResponse(_Base):
    positions: list[ForexPosition]
    account_id: str


class ForexPrice(_Base):
    instrument: str
    bid: FiniteFloat
    ask: FiniteFloat
    mid: FiniteFloat
...
class ForexPricesResponse(_Base):
    prices: list[ForexPrice]


class ForexInstrument(_Base):
    name: str                    # e.g. "EUR_USD"
    display_name: str            # e.g. "EUR/USD"
    pip_location: int            # e.g. -4 means 0.0001 per pip
    margin_rate: FiniteFloat


class ForexInstrumentsResponse(_Base):
    instruments: list[ForexInstrument]


class ForexOrderResult(_Base):
    order_id: str
    status: str                  # "FILLED" | "PENDING" | "CANCELLED" | "REJECTED"
    instrument: str
    units: int                   # positive=buy, negative=sell
    fill_price: FiniteFloat | None = None
    time_in_force: str
    submitted_at: str | None = None
    error: str | None = None


class ForexPositionCloseResult(_Base):
    instrument: str
    closed: bool
    long_units_closed: int
    short_units_closed: int
    realized_pl: FiniteFloat | None = None
    error: str | None = None
